...
import { computed, defineProps, reactive, useAttrs, watch } from 'vue';
...
function syncValue(file: EuUploadFile, fileList: EuUploadFile[], isRemove = false) {
  ...
  model.value = state.files
    .filter((item) => item.response && item.response.link)
...
<template>
  ...
  >
    ...
    <template #default>
      <!-- 图片预览 -->
      <el-image-viewer v-if="state.previewShow" :url-list="[state.previewUrl]" @close="state.previewShow = false" />
    </template>
  </el-upload>
</template>
